The following Project is a Task of the Odin Project, a open source Learningplatform for coding. See the original Task: 
...
We will approach this by the following Way;
. Remove contradicting Lines of Code from the previous RPS Task.
. Define Functions that are necessary
. Implement a eventLister for Clicks and 4 Buttons(Start, Rock, Scissor, Paper)
. Implement a function that shows a text on the Display and asks the User to chose when start is clicked.
. Run the game function with the correct input of the selection.
. Return the winner of each round and display a Score.
. If statement for Wincondition (5 Wins)
. Base Css
. 
